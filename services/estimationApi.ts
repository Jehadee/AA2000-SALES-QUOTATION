...
export async function fetchEstimationFiles(): Promise<EstimationFileRecord[]> {
  ...
  if (!baseUrl) {
    ...
    throw new Error(
      `Missing SERVER_API_URL. The estimation list fetch would call relative URLs on Vercel (HTML instead of JSON). ` +
        `Set SERVER_API_URL to your backend origin (e.g. "https://your-server-host.com") and ensure VITE_API_BASE_PATH matches.`
    );
  }
